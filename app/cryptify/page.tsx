...
export default function CryptifyPage() {
  ...
  useEffect(() => {
    ...
    try {
      const result = mode === 'encrypt' ? cryptifyCipher(inputText) : cryptifyDecipher(inputText);
      ...
      <div className="flex-1 flex flex-col p-4 md:p-6 gap-4 max-w-7xl mx-auto w-full">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl md:text-3xl font-bold text-black dark:text-white">Cryptify</h1>
            <div className="flex gap-1 p-1 bg-gray-100 dark:bg-gray-900 rounded-lg">
              <Button
                onClick={() => setMode('encrypt')}
                variant="ghost"
                size="sm"
                className={`${mode === 'encrypt' ? 'bg-black text-white dark:bg-white dark:text-black' : 'text-gray-600 dark:text-gray-400'} px-4 py-1 text-xs font-semibold transition-all`}
              >
                Encrypt
              </Button>
              <Button
                ...
                variant="ghost"
                size="sm"
                className={`${mode === 'decrypt' ? 'bg-black text-white dark:bg-white dark:text-black' : 'text-gray-600 dark:text-gray-400'} px-4 py-1 text-xs font-semibold transition-all`}
              >
                Decrypt
              </Button>
            </div>
